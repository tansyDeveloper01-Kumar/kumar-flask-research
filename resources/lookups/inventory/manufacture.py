...
class LookupInvManufacture(Resource):
    def get(self):
        try:
            ...
            if not sproc_result:
                return error_response(code=404, type='Not Found', message='manufacture not found', fbtrace_id=None), 404
            dropdown_data = [{'brand_id': each_product[0], 'brand': each_product[1]} for each_product in sproc_result]
            
            return {'status': 'Success', 'manufacture': dropdown_data}, 200
